<script>
	...
	async function loadStats() {
		try {
			...
			stats.total = allStats.by_type?.extra || 0;
			
			// Load active/inactive counts for extras only
			const activeResponse = await getModifiers({ type: 'extra', is_active: true });
...
<svelte:head>
	<title>Additions - Admin Panel</title>
</svelte:head>
...
<div class="p-6">
	...
	<div class="mb-6 flex items-center justify-between">
		<div>
			<h1 class="text-3xl font-bold text-gray-900">Additions</h1>
			<p class="text-gray-600 mt-1">Manage product additions and extras</p>
		</div>
		<button
	...
	<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
		<div class="bg-white rounded-lg shadow p-6">
			<div class="flex items-center justify-between">
				<div>
					<p class="text-sm font-medium text-gray-600">Total Additions</p>
	...
	<div class="bg-white rounded-lg shadow overflow-hidden">
		...
		{:else if modifiers.length === 0}
			<div class="p-12 text-center">
				...
				<p class="text-xl font-medium text-gray-900 mt-4">No additions found</p>
				<p class="text-gray-600 mt-2">Create your first addition to get started</p>
				<button
					on:click={openCreateModal}
		...
		<div class="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-screen overflow-y-auto">
			<div class="p-6 border-b border-gray-200">
				<h2 class="text-2xl font-bold text-gray-900">
					{editingModifier ? 'Edit Addition' : 'Add New Extra'}
				</h2>
			</div>
			...
			<form on:submit|preventDefault={handleSubmit} class="p-6 space-y-4">
				...
				<div>
					...
					<p class="text-sm text-gray-500 mt-1">Use 0 for free extras</p>
				</div>
				...
				<div class="flex items-center justify-end gap-4 pt-4 border-t border-gray-200">
					...
					>
						{editingModifier ? 'Update' : 'Create'} Extra
					</button>
				</div>
			</form>
		</div>
